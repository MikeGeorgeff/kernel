...
- `Profiler\Profiler` — aggregation layer behind `Kernel::getDebugInfo()`. Tracks any number of named `Profiler\Profile` instances (`initProfile()`, `hasProfile()`, `getProfile()`, `removeProfile()`) and lets any `DebuggableInterface` service `register()` itself under a name to be merged into the combined debug info output, replacing the kernel's previous hand-assembled `getDebugInfo()` array
