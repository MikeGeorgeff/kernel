...
- `Contract\ResettableInterface` (`reset(): void`) and `Kernel::resetShared(int $failureThreshold = 3, string ...$tags): static` — resets resolved shared services back to their original state; auto-detected via a post-resolution hook, no manual tagging required; intended for long-running processes (workers/daemons) wanting a clean slate between units of work. Passing one or more tags scopes the reset to only the resettable services carrying at least one of them (a service tagged with more than one requested tag is still only reset once); with no tags, every tracked service is reset, same as before tags were added. Internally resolved via the new `TagRegistryInterface::getTaggedIds()` (see below) into a set of container ids, then passed through to `DI\ServiceResetter::reset(int $failureThreshold = 3, ?array $ids = null)` — `null` means no filter (reset everything), a real (possibly empty) array means reset exactly that set. A tag that matches nothing currently trackable — never tagged anywhere, or tagged but not yet resolved — is a silent no-op, not an error; a typo'd tag name will not raise anything
- `TagRegistryInterface::getTaggedIds(string $tag): string[]` — returns the container ids registered under a tag without resolving them, unlike `getTagged()`; used internally by `resetShared()`'s tag-scoping, also available directly
- `Contract\ThresholdAwareResettableInterface extends ResettableInterface` (`getFailureThreshold(): int`) — lets an individual service override the default failure threshold passed to `resetShared()`
- `ServiceResetException` — a single `resetShared()` call runs every tracked service's `reset()` to completion regardless of earlier failures (one service breaching its threshold does not stop the rest from being attempted); once the pass finishes, if one or more services breached their threshold during that call, one `ServiceResetException` is thrown aggregating every breach — its message lists each failed service id alongside its exception message, and `getPrevious()` returns the first breach's original exception. Failures are tracked per container id (not per class, so the same class backing two different ids is tracked independently) and accumulate across separate calls to `resetShared()` until a successful reset clears that service's history
...
- `Kernel::getContainer()` and `Kernel::resetShared()` now throw `KernelException` if called after `shutdown()`, in addition to their existing before-boot guards
- `Profiler\Profiler` — aggregation layer behind `Kernel::getDebugInfo()`. Tracks any number of named `Profiler\Profile` instances (`initProfile()`, `hasProfile()`, `getProfile()`, `removeProfile()`) under a `profiles` key, and lets any `DebuggableInterface` service `register()` itself under a name to be merged into the output under a separate `components` key (kept apart from `profiles` so a registered component name can never collide with a profile name), replacing the kernel's previous hand-assembled `getDebugInfo()` array. `components` (but not `profiles`, which is built entirely from this package's own internal timing data and can't contain anything unsafe) is sanitized before being returned, defensively enforcing `DebuggableInterface::getDebugInfo()`'s "safe to print or serialize" contract even if an implementation doesn't honor it: objects → their class name, `Closure` instances → a `Closure#<spl_object_id>` reference string, backed enum cases → their `->value`, non-backed enum cases → their `->name`, resources → `get_resource_type()`, arrays → sanitized recursively. Enum handling must check `\BackedEnum` before `\UnitEnum` before the generic object fallback (`BackedEnum extends UnitEnum`, and any enum is also caught by a plain `is_object()` check) — the wrong order makes a later arm unreachable, which PHPStan correctly flags as `instanceof.alwaysFalse`; the class references must also be fully qualified (`\BackedEnum`, `\UnitEnum`) since they're global-namespace interfaces — an unqualified reference resolves against the current namespace instead and PHPStan reports `class.notFound`, not just an always-false check
- `Profiler\Phase` (`@internal`) — a single start/stop timer, tracking both wall-clock duration (`start()`/`stop()`/`getStartTime()`/`getEndTime()`/`getDuration()`) and memory usage (`getStartMemory()`/`getEndMemory()`/`getMemoryUsage()`, sampled via `memory_get_usage(false)` at the same moments as the timer). `isIncomplete(): bool` reports whether both a start and an end have been recorded; every timing/memory getter returns `null` while incomplete instead of a sentinel value
- `Profiler\Profile` (this class was `Debug\Profiler` — see the rename below) — now internally backed by one `Phase` for its own global timer plus one `Phase` per named phase (`startPhase()`/`stopPhase()`), instead of hand-rolling start/end timestamps itself. Gained `getName()`, `getEndTime()`, and `hasPhase(string $phase): bool`; `getDebugInfo()` now includes a `memory.usage` key (from the global timer) alongside the existing `start.time`/`end.time`/`duration`/`phases` keys, and each entry under `phases` is a full `Phase::getDebugInfo()` (`start.time`/`end.time`/`duration`/`memory.usage`), not just `duration`/`memory.usage`
- `ProfilerException` — thrown by `Profiler::getProfile()` when asked for a profile name that hasn't been initialized
- `DI\Profile\ServiceResolutionProfile`/`DI\Profile\ServiceProfile` (`@internal`), replacing `Debug\ServiceResolution`/`Debug\ResolvedService` — see the `Removed` entry below. Brings back per-resolution timing using the `onResolving`/`onResolved` hook pair on `ContainerBuilderInterface` (the design item flagged as unstarted when `resolutionTime` was dropped in 1.9.0): `Kernel::boot()`'s `containerInit` phase now pairs `onResolving` (starts a `Profiler\Phase`) with `onResolved` (stops it) per service id, so each resolution's duration and memory usage are captured, not just a resolution count. A shared service resolved multiple times (repeat `get()` calls hitting the container's cache) is tracked once; a non-shared service resolved multiple times accumulates one `Phase` per resolution. `ServiceProfile::getDebugInfo()` reports `count`, `duration` (summed across all resolutions), `memory` (summed), the full per-resolution breakdown under `resolutions` (each a `Phase::getDebugInfo()`), and a `debug.info` key when the resolved instance implements `DebuggableInterface` — replacing the old flat `resolutionCount`/`debugInfo` shape
- `DefinitionRepository::getInstropectionData(): array<string, array{shared: bool, aliases: string[], tags: string[]}>` — replaces `getRaw()` (see `Removed`); feeds `ServiceResolutionProfile`'s constructor
...
- **Breaking:** the `-INF` sentinel is dropped entirely from the profiler API in favor of `null` for "not yet available" — `Profiler\Profile::getStartTime()`, `getEndTime()`, `getOverallDuration()`, and `getPhaseDuration()` all return `?float` now instead of always returning `float`. `KernelInterface::getStartTime()`/`Kernel::getStartTime()` follow suit (return type widened from `float` to `?float`); `Kernel::getStartTime()` still checks `hasProfile('boot')` before calling `getProfile('boot')`, so it safely returns `null` (not `-INF`) when debug mode is enabled but `boot()` hasn't run yet, instead of throwing `ProfilerException`
...
- `Debug\ServiceResolution` and `Debug\ResolvedService` — replaced by `DI\Profile\ServiceResolutionProfile`/`DI\Profile\ServiceProfile` (see `Added`); the `Debug\` namespace under `src/` no longer exists
- `DefinitionRepository::getRaw()` — replaced by `getInstropectionData()` (see `Added`), which drops the `factory` key (never consumed by any caller) and adds a `tags` key
