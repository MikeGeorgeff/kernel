...
- `Contract\ResettableInterface` (`reset(): void`) and `Kernel::resetShared(int $failureThreshold = 3): static` — resets resolved shared services back to their original state; auto-detected via a post-resolution hook, no manual tagging required; intended for long-running processes (workers/daemons) wanting a clean slate between units of work
